...
int main( int argc, char* argv[] ) {
  ...
  for( std::set<MT2Region>::iterator iR=regions.begin(); iR!=regions.end(); ++iR ) {
     ...
     float N_llep_CR = 0.;//this_llepCR->Integral();
...
//	     }
	 ...
	 N_llep_CR= round(this_llepCR->GetBinContent(iBin));
	 ...
	 datacard << "llep_CRstat_" << gammaConvention( yield_llep, round(N_llep_CR), 2, binName, binName, (Rllep>0) ? ( (Rllep>3) ? 2 : Rllep ) : lastR_llep ) << std::endl;
...
//  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals           = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T2tt_eth.root", "T2tt", "isr" );
  ...
  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_lepEff    = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth.root", "T1tttt_sigcontam", "lepeff" );
  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_bTagHeavy = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth.root", "T1tttt_sigcontam", "btagsf_heavy" );
  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_bTagLight = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth.root", "T1tttt_sigcontam", "btagsf_light" );
  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_isr       = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth.root", "T1tttt_sigcontam", "isr" );
  std::vector<MT2Analysis<MT2EstimateSigContSyst>*> signals       = MT2Analysis<MT2EstimateSigContSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth.root", "T1tttt_sigcontam", "isr" );
...
//	}
	...
	for( int iBinY=1; iBinY<this_signalParent->GetNbinsX()+1; ++iBinY ){
	  ...
	  for( int iBinZ=1; iBinZ < iBinY; ++iBinZ ) {
		...
		Long_t modtime;
		Long_t size;
		std::string fullPathSE = Form("/pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1tttt_25Jan_binBybin/%s_%.0f_%.0f/datacard_%s_%s_%.0f_%.0f.txt", pathSE.c_str(), mParent, mLSP, binName.c_str(), sigName.c_str(), mParent, mLSP);
...
//////		std::string helpDatacard( Form("%s/datacard_%s_%s_forSed.txt", path_mass.c_str(), binName.c_str(), sigName.c_str()) );
		...
		std::string mkdirOnSE( Form("env --unset=LD_LIBRARY_PATH gfal-mkdir -p srm://t3se01.psi.ch:8443/srm/managerv2?SFN=/pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1tttt_25Jan_binBybin/%s_%.0f_%.0f", pathSE.c_str(), mParent, mLSP) );
		std::string copyOnSE( Form("xrdcp -v %s root://t3dcachedb.psi.ch:1094///pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1tttt_25Jan_binBybin/%s_%.0f_%.0f/datacard_%s_%s_%.0f_%.0f.txt", newDatacard.c_str(), pathSE.c_str(), mParent, mLSP, binName.c_str(), sigName.c_str(), mParent, mLSP) );
