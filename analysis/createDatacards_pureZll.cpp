...
int main( int argc, char* argv[] ) {
 ...
 //float err_zinv_corr   = 0.21; // 20% on Z/gamma ratio plus added in quadrature syst on templates (2%) and on f (4%) and MC stat on Rzg (5%) -> sqrt( 20*20 + 2*2 + 4*4 +5*5 ) = 21
  ...
  if( useMC_zinv )
    ...
    zinv        = MT2Analysis<MT2Estimate>    ::readFromFile( Form("Zll_CR_%s_%s_%.0ffb/MT2ZinvEstimate.root", samplesName.c_str(), regionsName.c_str(), lumi), "ZinvEstimate");
    zinv_ratio  = MT2Analysis<MT2Estimate>    ::readFromFile( Form("Zll_CR_%s_%s_%.0ffb/MT2ZinvEstimate.root", samplesName.c_str(), regionsName.c_str(), lumi), "ZllRatio");
    //  zinv_ratio  = MT2Analysis<MT2Estimate>    ::readFromFile( Form("Zll_CR_%s_%s_%.0ffb/data.root", samplesName.c_str(), regionsName.c_str(), lumi), "alpha");
    zll  = MT2Analysis<MT2Estimate>    ::readFromFile( Form("Zll_CR_%s_%s_%.0ffb/MT2ZinvEstimate.root", samplesName.c_str(), regionsName.c_str(), lumi), "Zll");
 
    /*
    zinv        = MT2Analysis<MT2Estimate>    ::readFromFile( Form("Zll_CR_%s_%s_%.0ffb/MT2ZinvEstimate.root", samplesName.c_str(), regionsName.c_str(), lumi), "ZinvEstimate");
    zinv_ratio  = MT2Analysis<MT2Estimate>    ::readFromFile( Form("Zll_CR_%s_%s_%.0ffb/MT2ZinvEstimate.root", samplesName.c_str(), regionsName.c_str(), lumi), "alpha");
    zll  = MT2Analysis<MT2Estimate>    ::readFromFile( Form("Zll_CR_%s_%s_%.0ffb/MT2ZinvEstimate.root", samplesName.c_str(), regionsName.c_str(), lumi), "Zll");
    */
    ...
    */
	...
	if( iR->nBJetsMin()<2 ) { // 0 and 1 btag
	  ...
	  // datacard << "zinv_ZGratio lnN   - " << 1.+err_zinv_corr << " - -" << std::endl;
	   ...
	   //FROM THE ZLL/Znunu RATIO
	   int Nll = round(this_zll->GetBinContent(iBin));
	   datacard << "zinv_Zll_stat_" << binName << " gmN "<< Nll << " - " << this_zinv_ratio->GetBinContent(iBin) << " - -" << std::endl;
