...
//  analyses_bg.push_back( MT2Analysis<MT2Estimate>::readFromFile(firstInputFile.c_str(), "DYJets") );
//  analyses_bg[3]->setName("DY+jets");
...
//    std::cout << "also known as: " << iHT->getNiceName().c_str() << std::endl;
//
...
//    ofs_file << "\\caption{Background and signal yields for " << iHT->getNiceName().c_str() << ", integrated over \\mttwo. The yields are normalized to \\lumival.}" << std::endl;
...
//  } //loop over HT regions
  ...
  std::string ofs_tr = dir + "/yieldtable_fullMC.tex";
