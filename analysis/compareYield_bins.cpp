...
int main() {
  ...
  std::string outputdir = "./YieldComparison_lostLepton_binned/";
  ...
  analysisFirst_ext->setName("Data-driven (Std.)");
...
void drawYields( const std::string& outputdir, MT2Analysis<MT2Estimate>*  analysisFirst_ext, MT2Analysis<MT2Estimate>*  analysisFirst_bin, std::vector< MT2Analysis<MT2Estimate> *> bgYields ) {
  ...
  TH1D* hdata_bin = new TH1D("hdata_bin", "", 185, 0, 185);
  ...
  TH1D* hestimate = new TH1D("hestimate", "", 185, 0, 185);
...
//	yerr=y*sqrt(yerr2);
	...
	h2_axes->SetXTitle("H_{T} [GeV]");
      ...
      legend->AddEntry( g_first, "data-driven (std.)", "P" );
  ...
  hestimate->GetXaxis()->SetLabelSize(0.035);
  //  hestimate->Draw("PE");
  ...
  legend->SetTextFont(42);
  legend->SetFillColor(0);
  legend->AddEntry( hdata, "data-driven (std.)", "PL" );
