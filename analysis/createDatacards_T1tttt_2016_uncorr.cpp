...
int main( int argc, char* argv[] ) {
  ...
  for( std::set<MT2Region>::iterator iR=regions.begin(); iR!=regions.end(); ++iR ) {
     ...
     if( iR->nJetsMax()==1 ){
       ...
       if ( ( iR->htMin()>=1500. && iR->nBJetsMin()==3 ) ){

	 MT2Region* thisTempRegion = new MT2Region(iR->htMin(), iR->htMax(), 7, -1, 2, 2);

	 this_qcdCR = qcdCR->get(*thisTempRegion)->yield;
	 this_qcd_ratio = qcd_ratio->get(*thisTempRegion)->yield;
	 this_qcd_purity = qcd_purity->get(*thisTempRegion)->yield;
	 this_qcd_ratioSystFit = qcd_ratioSystFit->get(*thisTempRegion)->yield;
...
//       /////
       ...
       datacard << "lumi_syst_2016    lnN    " << 1.+err_lumi_corr << " - - -" << std::endl;
       datacard << "sig_MCstat_2016_" << binName << " lnN UUU - - -" << std::endl;
       datacard << "sig_isrSyst_2016 lnN III - - -" << std::endl;
       datacard << "sig_syst_2016_" << binName << " lnN 1.15 - - -" << std::endl;
//       datacard << "sig_bTagHeavySyst lnN HHH - - -" << std::endl;
//       datacard << "sig_bTagLightSyst lnN LLL - - -" << std::endl;
//       datacard << "sig_lepEffSyst lnN EEE - - -" << std::endl;
...
//	      thisErrHT = ( this_zllG_ht->GetBinContent(thisBinHT) > 0 )     ? this_zllG_ht->GetBinError(thisBinHT)    / this_zllG_ht->GetBinContent(thisBinHT)     : 1.0;
	     ...
	     thisErrHTDn = ( this_zllG_ht_monojet->GetBinContent(thisBinHT) > 0 )     ? ( this_zllG_ht_monojet->GetBinContent(thisBinHT)     - thisDn_zllG_ht_monojet->GetBinContent(thisBinHT) )    / this_zllG_ht_monojet->GetBinContent(thisBinHT)     : 1.0;
           ...
           datacard << "zinv_doubleRatioOffset_2016 lnN   - " << 1.+err_zinv_doubleRatioOffset << " - -" << std::endl;
...
//           datacard << "zinv_ZGratio_" << zinvCR_name << " lnN   - " << 1.+err_ZGUp << "/" << 1.-err_ZGDn << " - -" << std::endl;
           ...
           datacard << "zinv_ZGratio_2016_" << htName << " lnN   - " << 1.+thisErrHTUp << "/" << 1.-thisErrHTDn << " - -" << std::endl;
           datacard << "zinv_ZGratio_2016_" << jName << " lnN   - " << 1.+thisErrNJUp << "/" << 1.-thisErrNJDn << " - -" << std::endl;
           datacard << "zinv_ZGratio_2016_" << bName << " lnN   - " << 1.+thisErrNBUp << "/" << 1.-thisErrNBDn << " - -" << std::endl;
	 ...
	 if( (!use_extrapolation && iR->nBJetsMin()>=2) || iR->nBJetsMin()>3 ) {
	   ...
	   datacard << "zinv_MC_2016_" << binName << " lnN - " << 1.+err_zinv_uncorr_2b << " - -" << std::endl;
...
//	   p_errDown = this_zinv_purity->GetErrorYlow ( 0 ); 
	   ...
	   if( Ngamma>0 && p>0 ) {
	     ...
	     datacard << "zinv_puritySyst_2016_" << zinvCR_name << " lnN  - " << 1.+err_zinv_puritySyst << " - -" << std::endl;
	     ...
	     if( fabs(p_errDown)/p < 1. )  
	       datacard << "zinv_purity_2016_" << zinvCR_name << " lnN  - " << 1.+p_errUp/p << "/" << 1.-p_errDown/p << " - -" << std::endl;
	     else 
	       datacard << "zinv_purity_2016_" << zinvCR_name << " lnN  - " << 1.+p_errUp/p << "/0.01 - -" << std::endl;
	     ...
	     datacard << "zinv_purity_2016_" << zinvCR_name << " lnN  - 1.0/0.01 - -" << std::endl;
	   ...
	   if( !use_extrapolation )
	     datacard << "zinv_CRstat_2016_" << std::setprecision(5) << gammaConvention( yield_zinv, Ngamma, 1, binName, binName, (R>0) ? ((R>0.5) ? 0.5 : R) : lastR_zinv ) << std::setprecision(3) << std::endl;
	   else {
	     datacard << "zinv_CRstat_2016_" << std::setprecision(5) << gammaConvention( yield_zinv, Ngamma, 1, zinvCR_name, binName, (R>0) ? ((R>0.5) ? 0.5 : R) : lastR_zinv ) << std::setprecision(3) << std::endl;
	     if( nBins>1 ){
	       if( iBin==1 && fabs(yield_zinv)>0 ){
		 
		 datacard << "zinv_shape_2016_" << zinvCR_name << " lnN  - " << 1.-shapeErr_zinv/fabs(yield_zinv) << " - - " << std::endl;
		 ...
		 datacard << "zinv_shape_2016_" << zinvCR_name << " lnN  - " << 1.+relativeErr << " - - " << std::endl;
	   ...
	   datacard << "zinv_alphaErr_2016_" << binName << " lnN  - " << 1.+alphaErr << " - -" << std::endl;
	 ...
	 datacard << "llep_lepeff_2016_" << llepCR_name << "  lnN  - - " << 1.+err_llep_lepEff << " -" << std::endl;
	 llep_systUp += err_llep_lepEff*err_llep_lepEff;
	 llep_systDn += err_llep_lepEff*err_llep_lepEff;
	 
	 datacard << "llep_CRstat_2016_" << gammaConvention( yield_llep, round(N_llep_CR), 2, llepCR_name, binName, (Rllep>0) ? ( (Rllep>3) ? 2 : Rllep ) : lastR_llep ) << std::endl;
...
//	   datacard << "llep_bTag_" << llepCR_name << " lnN - - 1.2 -" << std::endl;
	 ...
	 if( yield_llep>=0. ) {
	   ...
	   datacard << "llep_MCstat_2016_" << binName << " lnN  - - " << 1.+err_llep_mcstat << " -" << std::endl;
	   ...
	   if( nBins > 1 ){
	     if( iBin==1 && yield_llep>0 ){
	       ...
	       datacard << "llep_shape_2016_" << llepCR_name << " lnN - - " << 1.-shapeErr_llep/yield_llep << " - " << std::endl;
	       ...
	       datacard << "llep_shape_2016_" << llepCR_name << " lnN - - " << 1+relativeErr_llep << " - " << std::endl;
	   ...
	   if( iR->htMin()==200 && iR->nJetsMin()>=7 ){
	     ...
	     datacard << "llep_alpha_2016_" << llepCR_name << " lnN - - " << 1.+0.4 << " - " << std::endl;
	     llep_systUp += 0.4*0.4;
	     llep_systDn += 0.4*0.4;
	   
	   } else if( iR->nJetsMin()>=7 && iR->nBJetsMin()>=3 ){

	     datacard << "llep_alpha_2016_" << llepCR_name << " lnN - - " << 1.+0.15 << " - " << std::endl;
             llep_systUp += 0.15*0.15;
             llep_systDn += 0.15*0.15;

	   } else{

	     datacard << "llep_alpha_2016_" << llepCR_name << " lnN - - " << 1.+0.1 << " - " << std::endl;
             llep_systUp += 0.1*0.1;
             llep_systDn += 0.1*0.1;
	 ...
	 if( iR->nJetsMax()==1 ){
	   ...
	   datacard << "qcd_CRstat_2016_" << std::setprecision(6) << gammaConvention( yield_qcd, NQCD_cr, 3, binName, binName, (thisRqcd>0) ? thisRqcd : lastR_qcd_mono ) << std::setprecision(3) << std::endl;
	   ...
	   datacard << "qcd_alphaErr_2016_" << binName << " lnN - - - " << 1.+thisRQCDErr_monojet << std::endl;
...
//	   /////
	   ...
	   datacard << "qcd_CRstat_2016_" << std::setprecision(6) << gammaConvention( yield_qcd, NQCD_cr, 3, binName, binName, (thisRqcd*thisFractions>0) ? thisRqcd*thisFractions : 1.0 ) << std::setprecision(3) << std::endl;   
	   ...
	   datacard << "qcd_FJRBsyst_2016_" << binName << " lnN - - - " <<  1.+thisFractionsErr  << std::endl;
...
//	   //////
	   ...
	   datacard << "qcd_RPHIstat_2016_" << htName << " lnN - - - " <<  1.+thisRqcdErr  << std::endl;
	   qcd_systUp += thisRqcdErr*thisRqcdErr;
	   qcd_systDn += thisRqcdErr*thisRqcdErr;
	 
	   datacard << "qcd_RPHIsyst_2016_" << htName << " lnN - - - " <<  1.+thisRFitVarErr  << std::endl;
...
//  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_lepEff    = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T2tt_eth.root", "T2tt", "lepeff" );
//  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_bTagHeavy = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T2tt_eth.root", "T2tt", "btagsf_heavy" );
//  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_bTagLight = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T2tt_eth.root", "T2tt", "btagsf_light" );
//  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_isr       = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T2tt_eth.root", "T2tt", "isr" );
//  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals           = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T2tt_eth.root", "T2tt", "isr" );

//  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_lepEff    = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth_2016.root", "T1tttt_sigcontam", "lepeff" );
//  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_bTagHeavy = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth_2016.root", "T1tttt_sigcontam", "btagsf_heavy" );
//  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_bTagLight = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth_2016.root", "T1tttt_sigcontam", "btagsf_light" );
  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_isr       = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth_2016.root", "T1tttt_sigcontam", "isr" );
  std::vector<MT2Analysis<MT2EstimateSigContSyst>*> signals       = MT2Analysis<MT2EstimateSigContSyst>::readAllSystFromFile( "./signalScansFromDominick/T1tttt_sigcontam_eth_2016.root", "T1tttt_sigcontam", "isr" );
  
//  std::cout << "Setting names: " << std::endl;
//  std::cout << "For analysis " << signals[0]->getName() << std::endl;
//
//  signals[0]->setName("T1tttt");
//  std::cout << "Set names." << std::endl;
//
//  signals_bTagHeavy[0]->setName("T1tttt");
//  std::cout << "Set names." << std::endl;
//  signals_bTagLight[0]->setName("T1tttt");
//  std::cout << "Set names." << std::endl;
//  signals_isr[0]->setName("T1tttt");
//  std::cout << "Set names." << std::endl;
//  signals_lepEff[0]->setName("T1tttt");
//  std::cout << "Set names." << std::endl;
  ...
  for( unsigned  isig=0; isig<signals.size(); ++isig ) {
    ...
    if(sigName=="T1tttt_sigcontam") sigName="T1tttt";
    //    std::cout << "Set sigName to " << sigName << std::endl;
...
//	TH3D* this_signal3d_lepEff_Up;
	...
	MT2EstimateSigSyst* thisSigSyst = signals_isr[0]->get(*iR);

	if( thisSigSyst->yield3d!=0 && thisSigSyst->yield3d_systUp!=0){
	  
	  this_signal3d_central      = (TH3D*) signals_isr[isig]->get(*iR)->yield3d->Clone();
...
//	}
	...
	for( int iBinY=1; iBinY<this_signalParent->GetNbinsX()+1; ++iBinY ){
	  ...
	  for( int iBinZ=1; iBinZ < iBinY; ++iBinZ ) {
	    ...
	    TH3D* this_signal3d_crsl = (TH3D*) signals[isig]->get(*iR)->yield3d_crsl->Clone();
	    TH1D* this_signal_crsl   = this_signal3d_crsl->ProjectionX("mt2_crsl", iBinY, iBinY, iBinZ, iBinZ);

	    TH1D* this_signal_alpha  = (TH1D*) signals[isig]->get(*iR)->yield_alpha->Clone();
	    
	    TH1D* this_signalContamination = (TH1D*) this_signal_alpha->Clone();
		...
		Long_t modtime;
		Long_t size;
		std::string fullPathSE = Form("/pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1tttt_28June_2016_uncorr/%s_%.0f_%.0f/datacard_%s_%s_%.0f_%.0f.txt", pathSE.c_str(), mParent, mLSP, binName.c_str(), sigName.c_str(), mParent, mLSP);
...
//////		std::string helpDatacard( Form("%s/datacard_%s_%s_forSed.txt", path_mass.c_str(), binName.c_str(), sigName.c_str()) );
		...
		// Signal Contamination
		float sig_minusCont = sig - (this_signalContamination->GetBinContent(iBin));
		sig_minusCont = (sig_minusCont>=0) ? sig_minusCont : 0.0;
		//
...
//		
//		float lepEffErr = this_signal3d_lepEff_Up->GetBinContent(iBin, iBinY, iBinZ);
//                lepEffErr = lepEffErr/sig;

		float totUncorrErr = 1.+sqrt(sigErr*sigErr+2*0.05*0.05+0.1*0.1); // MC stat + PDF + scales + JEC

		std::string mvCommand( Form("mv %s %s", newDatacard.c_str(), helpDatacard.c_str()) );
		std::string rmCommand( Form("rm -f %s", helpDatacard.c_str()) );
		
		//		sig_minusCont*=2.26355/2.26;
		
		//std::string sedCommand( Form("sed 's/XXX/%.3f/' %s > %s", sig, templateDatacard.c_str(), newDatacard.c_str()) );
		std::string sedCommand( Form("sed 's/XXX/%.3f/' %s > %s", sig_minusCont, templateDatacard.c_str(), newDatacard.c_str()) );
...
//                system( sedCommand_lepEffErr.c_str() );
		...
		std::string mkdirOnSE( Form("env --unset=LD_LIBRARY_PATH gfal-mkdir -p srm://t3se01.psi.ch:8443/srm/managerv2?SFN=/pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1tttt_28June_2016_uncorr/%s_%.0f_%.0f", pathSE.c_str(), mParent, mLSP) );
		std::string copyOnSE( Form("xrdcp -v %s root://t3dcachedb.psi.ch:1094///pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1tttt_28June_2016_uncorr/%s_%.0f_%.0f/datacard_%s_%s_%.0f_%.0f.txt", newDatacard.c_str(), pathSE.c_str(), mParent, mLSP, binName.c_str(), sigName.c_str(), mParent, mLSP) );
...
//		system( mvCommand.c_str() );
//
//		std::string sedCommand_lepEffErr( Form("sed 's/EEE/%.3f/g' %s > %s", lepEffErr, helpDatacard.c_str(), newDatacard.c_str()) );
//		system( sedCommand_lepEffErr.c_str() );
//
//		system( rmCommand.c_str() );
