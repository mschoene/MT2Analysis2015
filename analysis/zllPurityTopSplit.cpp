...
void drawSFvsOF( const std::string& outputdir, std::vector< MT2Analysis<MT2EstimateTree> *> bgYields, std::vector< MT2Analysis<MT2EstimateTree> *> bgYields_of ) {
  ...
  for( std::set<MT2Region>::iterator iMT2 = MT2Regions.begin(); iMT2!=MT2Regions.end(); ++iMT2 ) {
	...
	for(int k=0; k<3; k++){
	  ...
	  h1_bg->SetFillColor( colors[index]+k );
...
yMax = 0.3; 
	...
	for(int k=0; k<3; k++){
	  ...
	  h1_bg1->SetFillColor(855+k);
	  if(k==0){

	    legend->AddEntry( h1_bg1, "tt", "F" );
  	  }else if(k==1){
	    h1_bg1->SetFillColor(855+k);
	    legend->AddEntry( h1_bg1, "tt other", "F" );
  	  }else{   
	    h1_bg1->SetFillColor(855+k);
    ...
    */   
    std::string cut =  "(abs(Z_mass-91.19)<20)";
    std::string cut2 = "(abs(Z_mass-91.19)<10)";
 
    std::string cut3 = "(abs(Z_mass-91.19)<20&&nBJets<2)";
    std::string cut4 ="(abs(Z_mass-91.19)<10&&nBJets<2)";

    std::string cut_nJets3 = "(abs(Z_mass-91.19)<10&&nJets>2)";
