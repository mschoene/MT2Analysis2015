...
//	}
	...
	for( int iBinY=1; iBinY<this_signalParent->GetNbinsX()+1; ++iBinY ){
	  ...
	  for( int iBinZ=1; iBinZ < iBinY; ++iBinZ ) {
		...
		Long_t modtime;
		Long_t size;
		std::string fullPathSE = Form("/pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1tttt_27Janv2/%s_%.0f_%.0f/datacard_%s_%s_%.0f_%.0f.txt", pathSE.c_str(), mParent, mLSP, binName.c_str(), sigName.c_str(), mParent, mLSP);
...
//////		std::string helpDatacard( Form("%s/datacard_%s_%s_forSed.txt", path_mass.c_str(), binName.c_str(), sigName.c_str()) );
		...
		std::string mkdirOnSE( Form("env --unset=LD_LIBRARY_PATH gfal-mkdir -p srm://t3se01.psi.ch:8443/srm/managerv2?SFN=/pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1tttt_27Janv2/%s_%.0f_%.0f", pathSE.c_str(), mParent, mLSP) );
		std::string copyOnSE( Form("xrdcp -v %s root://t3dcachedb.psi.ch:1094///pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1tttt_27Janv2/%s_%.0f_%.0f/datacard_%s_%s_%.0f_%.0f.txt", newDatacard.c_str(), pathSE.c_str(), mParent, mLSP, binName.c_str(), sigName.c_str(), mParent, mLSP) );
