...
int main(int argc, char* argv[]){
  ...
  MT2Analysis<MT2EstimateTree>* data2 = MT2Analysis<MT2EstimateTree>::readFromFile( ("EventYields_data_2016_diPhoton_2017Feb05/diPhotonControlRegion/data.root") , "diPhoton_data");
  //  MT2Analysis<MT2EstimateTree>* data2 = MT2Analysis<MT2EstimateTree>::readFromFile( ("EventYields_data_diPhoton_nov08_incl/diPhotonControlRegion/data.root") , "diPhoton_data");
  data2->setColor( kData );
  data2->setFullName("Data 2016");
  ...
  bgYields3.push_back( sig_2 );

  std::vector<MT2Analysis<MT2EstimateTree>* > bgYields_diPhoton; 
  bgYields_diPhoton.push_back( diPhoton );

  std::vector<MT2Analysis<MT2EstimateTree>* > bgYields_gjets; 
  bgYields_gjets.push_back( gjets );

  std::vector<MT2Analysis<MT2EstimateTree>* > bgYields_qcd; 
  bgYields_qcd.push_back( qcd );
  ...
  std::vector<MT2Analysis<MT2EstimateTree>* > data2016Yields; 
  data2016Yields.push_back( data2 );
 ...
 // if(myTree.nVert > 0 && myTree.nJet30 >= 1 && myTree.zll_ht>250. && ((myTree.nJet30>1 && myTree.zll_mt2>200.)||myTree.nJet30==1) && myTree.nJet30FailId == 0 && myTree.zll_deltaPhiMin > 0.3 && ( (myTree.nJet30>1 && myTree.zll_ht<1000. && myTree.zll_met_pt>250.) || (myTree.nJet30>1 && myTree.zll_ht>=1000. && myTree.zll_met_pt>30.) || (myTree.nJet30==1 &&myTree.zll_met_pt>250.)) && myTree.zll_diffMetMht < 0.5*myTree.zll_met_pt && myTree.nlep==2 && (myTree.lep_pdgId[0]*myTree.lep_pdgId[1])<0 && ((myTree.nJet30==1 && myTree.jet_id[0]>=4) || myTree.nJet30>1) && myTree.lep_pt[0]>=25. && myTree.lep_pt[1]>=20. && myTree.Flag_HBHENoiseFilter>0 && myTree.Flag_HBHENoiseIsoFilter>0 && myTree.Flag_globalTightHalo2016Filter>0 && myTree.Flag_EcalDeadCellTriggerPrimitiveFilter>0 && myTree.Flag_goodVertices>0 && myTree.Flag_eeBadScFilter>0 && myTree.Flag_badMuonFilter>0 && myTree.Flag_badChargedHadronFilter>0 && (myTree.HLT_MuX_Ele12 || myTree.HLT_Mu8_EleX || myTree.HLT_Mu33_Ele33_NonIso || myTree.HLT_Mu30_Ele30_NonIso) && (abs(myTree.lep_pdgId[0]) != abs(myTree.lep_pdgId[1])) && ((abs(myTree.lep_pdgId[0])==11 && myTree.lep_tightId[0]> 0.5) || (abs(myTree.lep_pdgId[0])==13)) && ((abs(myTree.lep_pdgId[1])==11 && myTree.lep_tightId[1]> 0.5) || (abs(myTree.lep_pdgId[1])==13)) && myTree.jet_pt[0]<13000. && myTree.met_pt/myTree.met_caloPt < 5. );
  ...
  selection.push_back( " ( gg_nJets>0 &&  ( ptGamma0/h_mass) > 0.33) && ((ptGamma1/h_mass )> 0.25) && fabs(etaGamma0)<=1.4  && fabs(etaGamma1)<=1.4  && r9Gamma0 > 0.5 && r9Gamma1>0.5 && gg_nJets>=4 && gg_nBJets==0 && (h_pt/h_mass)<0.8 ");
  selection.push_back( " ( gg_nJets>0 &&  ( ptGamma0/h_mass) > 0.33) && ((ptGamma1/h_mass )> 0.25) && fabs(etaGamma0)<=1.4  && fabs(etaGamma1)<=1.4  && r9Gamma0 > 0.5 && r9Gamma1>0.5 && gg_nJets>=4 && gg_nBJets==1 && (h_pt/h_mass)<0.8 ");
  selection.push_back( " ( gg_nJets>0 &&  ( ptGamma0/h_mass) > 0.33) && ((ptGamma1/h_mass )> 0.25) && fabs(etaGamma0)<=1.4  && fabs(etaGamma1)<=1.4  && r9Gamma0 > 0.5 && r9Gamma1>0.5 && gg_nJets>=4 && gg_nBJets>=2 && (h_pt/h_mass)<0.8 ");

  sel_name.push_back("b=0");
  sel_name.push_back("b=1");
  sel_name.push_back("b>=2");

  
  //std::vector<TCanvas*> MT2DrawTools::drawSelections_fromTree( const std::string& saveName, const std::string& varName, std::vector<std::string>& selection, int nBins, float xMin, float xMax, std::string axisName, const std::string& units, const std::string& kinCuts, const std::string& topoCuts ) {

  dt.drawSelections_fromTree( "higgs"   , "hgg_mt2"   , selection, sel_name, 25, 0., 100., "M_{T2} with H", "GeV", cutsLabel, "#geq0j, #geq0b");
  dt.set_mc( &bgYields2 );
  dt.drawSelections_fromTree( "sig0"   , "hgg_mt2"   , selection, sel_name, 25, 0., 100., "M_{T2} with H", "GeV", cutsLabel, "#geq0j, #geq0b");
  dt.set_mc( &bgYields3 );
  dt.drawSelections_fromTree( "sig2"   , "hgg_mt2"   , selection, sel_name, 25, 0., 100., "M_{T2} with H", "GeV", cutsLabel, "#geq0j, #geq0b");
  dt.set_mc( &bgYields_diPhoton );
  dt.drawSelections_fromTree( "diPhoton"   , "hgg_mt2"   , selection, sel_name, 25, 0., 100., "M_{T2} with H", "GeV", cutsLabel, "#geq0j, #geq0b");

  // dt.set_mc( &bgYields_gjets );
  // dt.drawSelections_fromTree( "gjets"   , "hgg_mt2"   , selection, 50, 0., 100., "M_{T2} with H", "GeV", cutsLabel, "#geq0j, #geq0b");
 
 // dt.set_mc( &bgYields_qcd );
 //  dt.drawSelections_fromTree( "qcd"   , "hgg_mt2"   , selection, 50, 0., 100., "M_{T2} with H", "GeV", cutsLabel, "#geq0j, #geq0b");
