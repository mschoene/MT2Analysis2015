...
int main( int argc, char* argv[] ) {
  ...
  std::string outputdir = cfg.getEventYieldDir() + "/YieldComparison_dataMC_binned_post_oneMLfile";
...
//
//  analysesSignal.push_back( MT2Analysis<MT2Estimate>::readFromFil1e( sigPath + "/T1bbbb_eth.root", "T1bbbb") );
...
//  styleSig.push_back(1);
  ...
  for(int s=0; s<sigSize+sigContSize; ++s){
    ...
    hsig[s] = new TH1D(thisNameS.c_str(), "", 213, 0, 213);
    hsig[s]->Sumw2();
  ...
  TH1D* hestimate_all = new TH1D(Form("hestimate_all"), "", 213, 0, 213);
  hestimate_all->Sumw2();
  hestimate_all->GetYaxis()->SetTitle("Entries");
  
  TH1D* hestimate[bgSize];

  TH1D* hestimate_all_forRatio = new TH1D(Form("hestimate_all_forRatio"), "", 213, 0, 213);
...
////  htRegions.push_back("extreme H_{T}");
  ...
  // draw the "Post-fit background" text
  ...
  if(doLogRatio){
    ...
    //    h2_axes_ratio_0 = new TH2D("axes_ratio_0", "", 10, oldBin, thisBin, 10, 0.5, 1.5 );
    h2_axes_ratio_0 = new TH2D("axes_ratio_0", "", 10, oldBin, thisBin, 10, 0.0, 2.0 );
  ...
  if(doLogRatio){
    ...
    //    h2_axes_ratio_1 = new TH2D("axes_ratio_1", "", 10, oldBin, thisBin, 10, 0.4, 1.6 );
  ...
  if(doLogRatio){
    gPad->SetLogy();
    h2_axes_ratio_5 = new TH2D("axes_ratio_5", "", 10, oldBin, thisBin, 10+1, 0.1, 10.0 );
  }
  else
    //    h2_axes_ratio_5 = new TH2D("axes_ratio_5", "", 10, oldBin, thisBin, 10, 0., 2.5 );
...
//  TH1D* hNobs08 = new TH1D("hNobs08", "", 213, 0, 213);
//  for(int t=0; t<100; ++t)
