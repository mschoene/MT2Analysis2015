...
int main( int argc, char* argv[] ){
  ...
  std::string outputdir = "MT2bins_2CRevents";
...
//  TH1F* hmt2g[nGammaRegions];
  ...
  for( std::set<MT2Region>::iterator iR=regions.begin(); iR!=regions.end(); ++iR ) {
    ...
    std::cout << std::endl << "First hresholds for region " << iR->getName().c_str() << std::endl;
    ...
    for(int i=nBins+1; i>0; --i){
      ...
      if( hmt2z[s]->GetBinCenter(i) - binWidth/2 > mt2CR_threshold - binWidth/2 )
	continue;
      ...
      if(y+yqcd > 0.1 && yqcd/(y+yqcd) < 0.1  && yCR > 5){
