...
int main( int argc, char* argv[] ) {
  ...
  for( std::set<MT2Region>::iterator iR=regions.begin(); iR!=regions.end(); ++iR ) {
	...
	if( mt2Max>=0. )
      ...
      //if( !(iR->nJetsMax()==1) ) yield_qcd = yield_qcd/3.;
  ...
  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_bTagHeavy = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1qqqq_eth.root", "T1qqqq", "btagsf_heavy" );
  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_bTagLight = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1qqqq_eth.root", "T1qqqq", "btagsf_light" );
  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals_isr       = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1qqqq_eth.root", "T1qqqq", "isr" );
  std::vector<MT2Analysis<MT2EstimateSigSyst>*> signals           = MT2Analysis<MT2EstimateSigSyst>::readAllSystFromFile( "./signalScansFromDominick/T1qqqq_eth.root", "T1qqqq", "isr" );
	...
	for( int iBinZ=1; iBinZ < iBinY; ++iBinZ ) {
	  ...
	  for( int iBin=1; iBin<this_signal->GetNbinsX()+1; ++iBin ) {
		...
		binName = std::string( Form("%s_m%.0ftoInf", iR->getName().c_str(), mt2Min) );
	      ...
	      //std::string fullPathSE = Form( "/pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1qqqq_25Jan_lessqcd/%s_%.0f_%.0f/datacard_%s_%s_%.0f_%.0f.txt", pathSE.c_str(), mParent, mLSP, binName.c_str(), sigName.c_str(), mParent, mLSP);
		...
		continue;
	      ...
	      */	
	      ...
	      std::string mkdirOnSE( Form("mkdir ./datacards_T1qqqq_01April_doubleGamma/") );
	      std::string mkdirOnSE2( Form("mkdir ./datacards_T1qqqq_01April_doubleGamma/%.0f_%.0f", mParent, mLSP) );
	      std::string copyOnSE( Form("cp %s ./datacards_T1qqqq_01April_doubleGamma/%.0f_%.0f/datacard_%s_%.0f_%.0f.txt", newDatacard.c_str(), mParent, mLSP, binName.c_str(), mParent, mLSP) );
	     
	      std::cout << newDatacard.c_str() << std::endl;

	      std::cout << Form("./datacards_T1qqqq_01April_doubleGamma/%.0f_%.0f/datacard_%s_%.0f_%.0f.txt", mParent, mLSP, binName.c_str(), mParent, mLSP) << std::endl;
 

	      //	      std::string mkdirOnSE( Form("gfal-mkdir -p srm://t3se01.psi.ch:8443/srm/managerv2?SFN=/pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1qqqq_25Jan_lessqcd/%s_%.0f_%.0f", pathSE.c_str(), mParent, mLSP) );
	      //	      std::string copyOnSE( Form("xrdcp -v %s root://t3dcachedb.psi.ch:1094///pnfs/psi.ch/cms/trivcat/store/user/mmasciov/datacards_T1qqqq_25Jan_lessqcd/%s_%.0f_%.0f/datacard_%s_%s_%.0f_%.0f.txt", newDatacard.c_str(), pathSE.c_str(), mParent, mLSP, binName.c_str(), sigName.c_str(), mParent, mLSP) );
	      //system( mkdirOnSE.c_str() );
	      //system( mkdirOnSE2.c_str() );
	      system( copyOnSE.c_str() );

	      /*for(int c=0; c<3; ++c){

		checkFileSE = (int) gSystem->GetPathInfo( "./datacards_T1qqqq_01April/",&id, &size, &flags, &modtime);
